module FireRecord
  module Reflection
    ...
    module ClassMethods
      ...
      def belongs_to name
        if reflections.values.any? { |ref| ref.is_a?(FireRecord::Reflection::BelongsTo) }
          raise "Cannot belong to more than one model"
        end
        ref = define_one_association name, FireRecord::Reflection::BelongsTo
        define_inverse_association ref
      end

      def has_one name
        ref = define_one_association name, FireRecord::Reflection::HasOne
        define_inverse_association ref
      end
        ...
        ref.klass.define_method self.model_name.plural do
          ...
          proxy = Class.new(this) do
            ...
            define_singleton_method :scope do
              ::FireRecord.client.doc(doc_path).col(this.model_name.plural)
            end

            define_singleton_method :build do |**attrs|
              new({ref.name => ref_obj}.merge(attrs || {}))
            end
          end
          proxy
        end
